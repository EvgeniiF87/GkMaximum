...
import { setAddSearchResultIsShoppingCart } from "../../../store/reducers/SearchSlice";

type ProductFullInfoProps = {
  route: RouteProp<CatalogAndSearchStackParamList, "CatalogProductFullInfo">;
};

const CatalogProductFullInfo: FC<ProductFullInfoProps> = ({ route }) => {
  const { goBack, replace } = useNavigation<AppPropsScreen>();
  ...
  useEffect(() => {
    if (whatComeScreen && whatComeScreen === "search") {
...
export default CatalogProductFullInfo;
